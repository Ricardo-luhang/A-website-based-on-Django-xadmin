...
$(function(){
    //个人资料修改密码
    $('#jsusersResetPwd').on('click', function(){
    ...
    $('#jsEditusersBtn').on('click', function(){
        var _self = $(this),
            $jsEditusersForm = $('#jsEditusersForm')
            verify = verifySubmit(
            [
                {id: '#nick_name', tips: Dml.Msg.epNickName, require: true}
            ]
        ...
        $.ajax({
            ...
            dataType:'json',
            url:"/users/info/",
            data:$jsEditusersForm.serialize(),
            async: true,
            beforeSend:function(XMLHttpRequest){
                _self.val("保存中...");
            ...
            success: function(data) {
                ...
                }else if(data.mobile_num){
                   _showValidateError($('#address'), data.mobile_num);
                }else if(data.status == "failure"){
                     Dml.fun.showTipsDialog({
                        title: '保存失败',
                        h2: data.msg
